...
namespace CS3358_Sp2016
{
    ...
    void sequence::resize(size_type new_capacity)
    {
        ...
        value_type* newData = new(nothrow) value_type[capacity];
        for(size_type i = 0; i < used; ++i)
        {
            newData[i] = data[i];
        }
    ...
    void sequence::insert(const value_type& entry)
    {
       if(used == capacity)
       {
           size_type new_capacity = (capacity * 1.25) + 1;
           resize(new_capacity);
       }
    ...
    sequence& sequence::operator=(const sequence& source)
    {
        if(this != &source)
        {
            value_type* newData = new(nothrow) value_type[source.capacity];
    ...
    {
        assert(is_item() == true);
        value_type currValue = data[current_index];
        return currValue;
    }
}
